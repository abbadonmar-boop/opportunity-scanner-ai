...
Synchronize approved D-051 — Discord Message Lifecycle and PostgreSQL Persistence Contract — into the remaining authoritative project documentation and Git before creating or applying any Discord migration. Preserve the approved lifecycle contract: one current row per Discord message, stable identity, in-place MESSAGE_UPDATE with re-filtering, hard DELETE for MESSAGE_DELETE / MESSAGE_DELETE_BULK, fail-safe handling of incomplete unknown MESSAGE_UPDATE payloads, and no REST/history fallback without separate authorization. Preserve the three D-051 production gates: verified encryption-at-rest design for PostgreSQL and applicable backup / WAL / recovery paths, current privacy policy plus accessible user modification / deletion request mechanism, and confirmed required `MESSAGE_CONTENT` availability before live textual persistence. Real Discord persistence, live Gateway / REST access, token actions, guild installation and production activation remain NOT AUTHORIZED. Do not create or apply `003_discord_source_items.sql` until D-051 documentation synchronization is complete.
...
- [ ] MODULE 7 — X Collector — SCOPE APPROVED / PRE-IMPLEMENTATION / ACCESS GATED — formal D-033 X / Twitter status: AVAILABLE WITH LIMITATIONS under the D-046 reconciliation; official access path verified; D-034 through D-049 are FIXED / APPROVED; Module 7 is not PASS; D-048 records Batch Compliance `PRODUCTION SUITABILITY: NOT PROVEN`: current reviewed authoritative evidence does not establish a guaranteed worst-case lifecycle bound sufficient to prove the applicable 24-hour requirement will always be met, but this does not prove that Batch Compliance exceeds 24 hours; Compliance event streams remain a separate Enterprise mechanism requiring separate explicit approval; persistent production X collection remains `COMPLIANCE-GATED / NOT AUTHORIZED`; production cadence, backup / WAL / restore compliance and recurring operating cost remain unresolved; D-049 does not complete, remove, reorder or waive Module 7 and authorizes no additional X API activity, Enterprise activation, Compliance Stream connection, migration `003_x_source_items.sql`, PostgreSQL change, X Collector implementation, Staging connection or Production connection.
- [ ] MODULE 8 — Telegram Sources — BLOCKED — TELEGRAM CONTENT AI-USE TERMS CONFLICT
- [ ] MODULE 9 — Discord Collector — SCOPE APPROVED / IMPLEMENTATION IN PROGRESS — D-050 and D-051 are FIXED / APPROVED; Discord retains D-033 status `AVAILABLE WITH LIMITATIONS`; the first code-only / synthetic foundation remains verified in commit `35ee017`; D-051 fixes the Discord message lifecycle and logical PostgreSQL persistence contract, including stable identity, in-place edits with re-filtering, hard deletion, incomplete-update fail-safe, encryption-at-rest gate, privacy / deletion-request gate and `MESSAGE_CONTENT` fail-safe; Module 9 is not PASS; `003_discord_source_items.sql` must not be created or applied until D-051 documentation synchronization is complete; real Discord persistence, live Gateway / REST access, token actions, guild installation and production activation remain NOT AUTHORIZED.
