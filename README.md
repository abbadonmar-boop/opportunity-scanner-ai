...
- Module 9 — Discord Collector: SCOPE APPROVED / PRE-IMPLEMENTATION — D-050 is FIXED / APPROVED; Discord retains D-033 status `AVAILABLE WITH LIMITATIONS`; only official Discord Application / bot / guild installation / Gateway / documented API access is permitted; guild / channel allowlisting, least privilege, `MESSAGE_CONTENT` gating, rate-limit compliance and the AI-training prohibition are mandatory; code-only / synthetic foundation is authorized only after D-050 documentation synchronization; real credentials, real guild installation, live Gateway / REST access, Discord-specific PostgreSQL schema changes and persistence of real Discord message content remain NOT AUTHORIZED without separate controlled approval.
...
- Application code: IN PROGRESS — implemented through verified Module 6; Module 5 remains BLOCKED — DATA ACCESS NOT APPROVED; Module 7 remains SCOPE APPROVED / PRE-IMPLEMENTATION / ACCESS GATED and is not PASS; Module 8 remains BLOCKED and is not PASS; D-034 through D-050 are FIXED / APPROVED; formal X / Twitter source status remains AVAILABLE WITH LIMITATIONS; Batch Compliance production suitability remains `NOT PROVEN`, not proven unsuitable; persistent production X collection remains COMPLIANCE-GATED / NOT AUTHORIZED; Compliance event streams remain a separate Enterprise mechanism requiring separate approval; backup / WAL / restore compliance and recurring operating cost remain unresolved; D-049 permits controlled roadmap continuation to Module 9 without completing, removing, reordering or waiving Modules 7 or 8; D-050 approves Module 9 scope and authorizes only code-only / synthetic foundation after documentation synchronization; Module 9 is not PASS; real Discord credentials, guild installation, live Gateway / API access, Discord-specific PostgreSQL schema changes and persistence of real Discord message content remain NOT AUTHORIZED without separate controlled approval; no new X API activity, Enterprise activation, migration `003_x_source_items.sql`, PostgreSQL change or X Collector implementation is authorized.
