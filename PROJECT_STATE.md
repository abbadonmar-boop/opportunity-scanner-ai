...
## CURRENT STEP
D-051 — Discord Message Lifecycle and PostgreSQL Persistence Contract — is FIXED / APPROVED. D-051 fixes the logical `discord_source_items` lifecycle contract: one current row per Discord message, stable source identity / deduplication, in-place MESSAGE_UPDATE with re-filtering, hard DELETE for MESSAGE_DELETE / MESSAGE_DELETE_BULK, no retained tombstone or source-specific derived identity after deletion, fail-safe handling of incomplete unknown MESSAGE_UPDATE payloads, and no REST/history fallback without separate authorization. D-051 also fixes three production gates: verified encryption-at-rest design for PostgreSQL and applicable backup / WAL / recovery paths, an up-to-date privacy policy plus accessible user modification / deletion request mechanism, and fail-safe verification of required `MESSAGE_CONTENT` availability before live textual persistence. Real Discord persistence, live Gateway / REST access, token actions, guild installation and production activation remain NOT AUTHORIZED.
...
## NEXT STEP
Synchronize approved D-051 into the remaining authoritative project documentation and Git before creating or applying any Discord migration. Preserve the D-050 / D-051 boundaries: no live Discord calls, token actions, real guild installation, real Discord persistence or production activation. Do not create or apply `003_discord_source_items.sql` until D-051 documentation synchronization is complete. After synchronization, the next authorized implementation step is synthetic-only creation and verification of `003_discord_source_items.sql`.
