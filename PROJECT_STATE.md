...
## CURRENT MODULE
Module 9 — Discord Collector — SCOPE APPROVED / PRE-IMPLEMENTATION

## CURRENT STEP
D-050 — Module 9 Discord Collector Scope, Official-Access Boundary and Acceptance Criteria — is FIXED / APPROVED. Discord retains the D-033 status `AVAILABLE WITH LIMITATIONS`. Module 9 scope is limited to the official Discord access path and the source-side pipeline `Discord → COLLECT → NORMALIZE → early PostgreSQL persistence → DEDUPLICATE → existing Module 6 Filter Engine`. D-050 authorizes only code-only / synthetic implementation foundation after documentation synchronization. Real Discord credentials, guild installation, Gateway connections, live Discord API calls, Discord PostgreSQL schema changes and persistence of real Discord message content remain NOT AUTHORIZED.
...
## NEXT STEP
Synchronize approved D-050 into the remaining authoritative project documentation and Git. Preserve the official-access boundary, least-privilege requirements, `MESSAGE_CONTENT` gate, guild / channel allowlist requirement, rate-limit compliance, AI-training prohibition, separate Discord persistence decision and separate live-access authorization. Do not generate or expose Discord credentials, install the application into a real guild, open a real Gateway connection, make live Discord API calls, create Discord-specific PostgreSQL schema changes or begin live Discord collection during this documentation synchronization step.
